...
export const fetchPlanning = cache(async () => {
  ...
  const url = `https://sport.nubapp.com/api/v4/activities/activities-calendar/guests?app_version=5.10.02&id_application=89834312&start_timestamp=${format(
    firstDayOfWeek,
    "dd-MM-yyyy"
  )}&end_timestamp=${format(
    new Date(`12-31-${getYear(today) + 1}`),
    "dd-MM-yyyy"
